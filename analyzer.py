            ...
            for i in range(1, len(item_set)):
                for antecedent in combinations(item_set , i):
                    ...
                    if confidence >= min_confidence :
                        ...
                        if len(rules) >= 10: # only need 10 rules
                            break
...
def visualize_kmeans_clusters(df, cluster_labels):
    ...
    fig.update_layout(title_text="Customer Cluster Visualization")  # Update the title here
    return fig
...
def main():
    ...
    if not rules.empty:
        print("Apriori algo complete. Found {} rules.".format(len(rules)))
